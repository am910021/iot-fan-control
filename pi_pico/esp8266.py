...
class ESP8266:
    ...
    def _sendNoRecv(self, atCMD, delay=0.1):
        self.__txData=atCMD
        self.__uartObj.write(self.__txData+'\r\n')
        #print('_sendNoRecv cmd, ', atCMD)
        time.sleep(delay)
        while True:
            #print(".")
            if self.__uartObj.any()>0:
    ...
    def doHttpPost(self,host,path,user_agent,content_type,content,port):
        """
    ...
    def SerialRead(self):
        SerialRecv = ""
        SerialRecv=str(self.__uartObj.read(1))
    ...
    def command(self, cmd):
        retData = self._sendToESP8266(cmd+"\r\n")
        if(retData != None):
            if ESP8266_OK_STATUS in retData:
                #print(str(retData,"utf-8"))
                retData = str(retData).partition(r"OK")[0]
                ...
                return ESP8266_ERROR_STATUS
        else:
            return ESP8266_ERROR_STATUS
        
    def commandRetArr(self, cmd):
        retData = self._sendToESP8266(cmd+"\r\n")
        if(retData != None):
            if ESP8266_OK_STATUS in retData:
                #print(str(retData,"utf-8"))
    ...
    def read(self):
        ...
        if '+IPD' in self.__rxData:
            print(self.__rxData)
            
    def createHttpServer(self, file, port=80):
        self.command('AT+CIPMUX=1')
        self.command('AT+CIPSERVER=1,%d' % (port))
        log = self.commandRetArr('AT+CIFSR')
        addresses = []
        for i in log:
            if 'IP' in i:
                tmp = i.split(',')[1].replace('"','')
                addresses.append(tmp)
                print('Listening %s:%d' % (tmp, port))

    def checkClient(self):
        data = self._sendToESP8266('AT+CIPSTATUS'+"\r\n")
        if(data == None) or (ESP8266_OK_STATUS not in data):
            return -1
        
        data = str(data)
        if '+CIPSTATUS:' not in data:
            return -1
                
        index = data.index('+CIPSTATUS:')
        data = data[index:].split(r"\r\n")
        for i in data:
            if '+CIPSTATUS:' in i:
                #+CIPSTATUS:2,"TCP","192.168.4.2",56615,80,1
                index = i.index(',"TCP"')
                pid = int(i[11:index])
                return pid
    ...
    def waitClient(self):
        while True:
            ...
            if '+IPD' in self.__rxData:
                data = str(self.__rxData)
                index = data.index('+IPD')
                size = len(data)
                data = data[index:size-9]

                retData = data.split(r"\r\n")
                print('\r\n'.join(retData))
                pid = int(retData[0].split(',')[1])
                return pid
            
    def sendHtml(self,file, pid):
        HTMLFileObject = open (file, "r")
        HTMLFileLines = HTMLFileObject.readlines()
        HTMLFileObject.close()
        self._sendNoRecv('AT+CIPSEND=%d,17' % (pid))
        self._sendNoRecv('HTTP/1.1 200 OK')
        self._sendNoRecv('AT+CIPSEND=%d,25' % (pid))
        self._sendNoRecv('Content-Type: text/html')
        self._sendNoRecv('AT+CIPSEND=%d,19' % (pid))        
        self._sendNoRecv('Connection: close')
        self._sendNoRecv('AT+CIPSEND=%d,2' % (pid))
        self._sendNoRecv('')
        self._sendNoRecv('AT+CIPSEND=%d,17' % (pid))
        self._sendNoRecv('<!DOCTYPE HTML>')
        ...
        for line in HTMLFileLines:
                ...
                self._sendNoRecv('AT+CIPSEND=%d,%d' % (pid, ciplength))
                self._sendNoRecv(cipsend)
        self._sendNoRecv('AT+CIPCLOSE=%d' % (pid)) # once file sent, close connection
        time.sleep(0.5)
