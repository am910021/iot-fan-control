...
class Stream:
    ...
    def _canWrite(self, size=0):
        if self.length()+size > self._limit:
            raise StreamException("Stream out of range, available write size is {}.".format(self._limit - self.length()))
    ...
    def writeStr(self, wo:str):
        ...
        self._canWrite(size)
        if size > SHORT_MAX:
            raise StringOutOfRange("Out of the range, the maximum byte length of the utf8 string is {}.".format(SHORT_MAX))
